<template>
    <div class="bg-white rounded grap-10 overflow-hidden shadow-card">
      <router-link :to="{ name: 'single-course', params: { courseType: type, slug:course.slug}}">
        <img  :src="course.image_url" alt="PEST Driven Laravel" class="w-full h-auto object-contain">
      </router-link>
        <div class="px-6 pb-5">
          <div class="mt-7 flex justify-between items-center gap-4 mb-4">
            <router-link :to="{ name: 'single-course', params: { courseType: type, slug:course.slug}}" class="text-lg-primary w-full hover:underline">{{ course.title }}</router-link>
          </div>
          <div>
            <div class="flex items-center mb-2">
              <div class="flex items-center">
                <!-- <img src=""> -->
                <vue-gravatar class="object-contain flex shrink-0 rounded-full" :email="course.submitted_by.email" :size="30" />

              </div>
              <h4 class="text-sm-primary w-full leading-5 ml-2">{{ course.submitted_by.name }}</h4>
            </div>
            <div class="mt-3">
              <div class="flex items-center gap-3 mt-6">
                <p class="bg-parrot-100 text-sm-primary py-1.5 px-1.5 rounded-[3px] flex items-center gap-1.5 w-max">
                  ...
                  </svg> {{ course.duration == 0 ? '1-5 hours' :course.duration == 1 ? '5-10 hours' : '10+ hours'  }}
                </p>
